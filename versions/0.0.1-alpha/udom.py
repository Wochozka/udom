...
Conversion from UninstallView text files export to dokuwiki files and syntax.
...
def generateDirStructure():
    if(args.merge):
        ...
        if os.path.exists("./inventura") and os.path.exists("./inventura/computers") and os.path.exists("./inventura/software"):
            ...
            print("Can not merge current structure. Remove current structure and run again without -m parameter or repair.")
            print(
...
def main():
    ...
    if(args.file):
        ...
        #print(generateDokuSite(readSingleComp(args.source_file)))
        pass
    if(args.dir):
...
def init_args():
    ...
    #p.add_argument("source_file", help="source file")
    #p.add_argument("target_file", help="target file")

    p.add_argument("-m", "--merge", action="store_true", help="merge with current structure (or create new)")
    ...
    fileOrDir = p.add_mutually_exclusive_group(required=False)
    fileOrDir.add_argument("-f", "--file", default=True, help="source file")
